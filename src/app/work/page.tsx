          ...
          {regions.map((region) => (
            <button
              ...
              className={`rounded-full px-4 py-1.5 text-xs font-medium uppercase tracking-widest transition-colors ${
                activeRegion === region
                  ? "bg-accent-hover text-text-light"
                  : "border border-border-light text-accent-secondary hover:border-accent-secondary"
              }`}
            >
              {region}
            </button>
        ...
        {filtered.map((item) => (
          ...
          >
            ...
            <div className="absolute right-3 top-3 flex gap-1.5">
              ...
              <span className="meta-text rounded-full bg-bg-dark/80 px-3 py-1 text-xs text-text-light">
                {item.category}
              </span>
            </div>
          </div>
