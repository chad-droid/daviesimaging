...
// /shore2026 — the ModelMatch landing page for attendees of the Shore Sales and
// Marketing Leadership Summit (San Diego, July 15 to 17, 2026). Cloned from
// /modelmatch-datalibrary (same Hormozi cold-traffic architecture and the same
// free-trial offer, inline via MMTrialEmbed at #start), with light Summit
// framing: an attendee eyebrow, a margin-and-carry-cost hook that matches the
// Summit theme, and a "find us at the Summit" booth + session callout.
//
// Claim rules honored throughout (same as the source page): sequence not
// causation ("found a buyer after ModelMatch", never "sold it"), no prices in
// proof, no banned words (stunning / transform / proven / guaranteed), no comp
// claims, and no em dashes anywhere.
...
import { CASE_STUDIES } from "@/app/modelmatch-datalibrary/caseStudies";
...
@media (max-width: 600px) {
  ...
  .mm-lib .mm-presence { grid-template-columns: 1fr !important; gap: 16px !important; }
}
`;
...
// ── Small Summit pill, shown above the hero headline ─────────────
function SummitPill() {
  return (
    <span
      style={{
    ...
    >
      ...
      Shore Summit 2026 · Attendee Access
    </span>
  );
}
...
function Hero() {
  return (
    <section style={{ position: "relative", padding: "132px 40px 80px", background: CREAM }}>
      <div style={{ maxWidth: 1240, margin: "0 auto" }}>
        ...
        <Reveal delay={0.18}>
          ...
          >
            Every day a finished spec sits, it carries. ModelMatch stages your vacant spec homes with
            furniture from your own model homes, so the listing shows like your model instead of an
            empty room. Your first eight images are free, delivered in 1 business day.
          </p>
        </Reveal>
        <Reveal delay={0.26}>
...
// ── 1b. Find us at the Summit — booth + session ──────────────────
    ...
    <section style={{ background: ACCENT, color: "#fff", padding: "26px 40px" }}>
      <div style={{ maxWidth: 1240, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", flexWrap: "wrap", gap: 8, marginBottom: 20 }}>
          <p style={{ margin: 0, fontFamily: "var(--font-body)", fontSize: 12, fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", color: "rgba(255,255,255,0.85)" }}>
            Find us at the Summit
          </p>
          <p style={{ margin: 0, fontFamily: "var(--font-body)", fontSize: 13, color: "rgba(255,255,255,0.8)" }}>
            {SHORE.dates} · {SHORE.place}
          </p>
        </div>
        <div className="mm-presence" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
          {cards.map((c, i) => (
            <div key={i} style={{ borderTop: "1px solid rgba(255,255,255,0.25)", paddingTop: 16 }}>
              <p style={{ margin: 0, fontFamily: "var(--font-body)", fontSize: 11, fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", color: "rgba(255,255,255,0.7)" }}>
                {c.label}
              </p>
              <p style={{ margin: "8px 0 0", fontFamily: "var(--font-heading)", fontSize: 24, fontWeight: 600, letterSpacing: "-0.01em", color: "#fff", lineHeight: 1.2 }}>
                {c.head}
              </p>
              <p style={{ margin: "6px 0 0", fontFamily: "var(--font-body)", fontSize: 14.5, lineHeight: 1.5, color: "rgba(255,255,255,0.85)", maxWidth: 380 }}>
                {c.sub}
              </p>
            </div>
          ))}
...
export function Shore2026() {
  return (
    <div className="mm-lib" data-screen-label="Shore Summit 2026">
