...
- `MODELMATCH_PASSWORD` — Basic Auth password for the `/modelmatch-demo` showcase (username `dig`); middleware fails closed if unset

---

## ModelMatch Showcase (`/modelmatch-demo`)

Self-contained static single-page app (Pretty / Powerful / Platform) served from `public/modelmatch-demo/`,
gated by HTTP Basic Auth in `src/middleware.ts` (username `dig`, password = `MODELMATCH_PASSWORD`).
`/modelmatch-demo` rewrites to `/modelmatch-demo/index.html` in `next.config.ts`. Not in nav or sitemap.

Source project: `/Users/chadh/Claude/Projects/ModelMatch Before & After Demonstration/` (see its
`INTEGRATION_PLAN.md`). When re-copying a regenerated `_showcase-deploy/` build over `public/modelmatch-demo/`,
re-apply three local patches or images will 404 / load unscoped:
1. `<base href="/modelmatch-demo/">` tag in `index.html` head
...
live at `public/mm/` — they were moved out of `public/modelmatch-demo/` so the showcase could own that path.
Do not put public assets under `public/modelmatch-demo/`; everything there is behind the password gate.
